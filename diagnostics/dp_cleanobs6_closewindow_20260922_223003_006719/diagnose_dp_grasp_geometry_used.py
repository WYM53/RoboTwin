#!/usr/bin/env python3
"""Causal diagnostic: original action chunk + clean boundary observation history + non-working-arm masking.
    ...
    try:
        ...
        def monitored_eval(*args, **kwargs):
            ...
            rec.event(
                ...
                note="original chunk length retained; only duplicate final frame at chunk boundary is removed",
            )
            print(
                f"[DIAG] intervention: keep n_action_steps={model.runner.n_action_steps}; "
                "suppress duplicate boundary obs only",
                flush=True,
            )
...
def parent(opts) -> int:
    ...
    run = root / "diagnostics" / ("dp_cleanobs6_closewindow_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
