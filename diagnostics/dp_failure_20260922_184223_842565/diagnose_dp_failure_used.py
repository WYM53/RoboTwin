#!/usr/bin/env python3
"""Read-only runtime diagnostics for the uploaded RoboTwin robotwin_dp code.
...
Actions, TOPP arguments, control loop, observation history and success criteria
remain unchanged. Requires the user's existing RoboTwin dependencies.
...
class Recorder:
    def __init__(self, folder: Path, requested_seed: int):
        self.folder = folder
    ...
    def attach(self, env, model):
        ...
        def take_action(action, *args, **kwargs):
            ...
            row = {"action_step": int(env.take_action_cnt) + 1, "prediction": self.current_prediction,
                   "action": arr.copy(), "pre": pre, "topp": []}
            self.event("action_begin", action_step=row["action_step"], action=arr, state=pre)
            restored = []
            ...
            try:
                ...
                result = original_action(action, *args, **kwargs)
            except BaseException as exc:
                row["raised"] = repr(exc)
                raise
            finally:
...
def worker(opts) -> int:
    ...
    rec = Recorder(case, opts.worker_seed)
    code = 0
    try:
...
def parent(opts) -> int:
    ...
    run = root / "diagnostics" / ("dp_failure_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
    ...
    shutil.copyfile(Path(__file__).resolve(), run / "diagnose_dp_failure_used.py")
    ...
    try:
        ...
        print(f"\n诊断包：{archive}\n请上传这个 ZIP；其中含完整日志、逐步动作/关节记录和原流程生成的视频。", flush=True)
    return exit_code
