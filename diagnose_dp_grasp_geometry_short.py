#!/usr/bin/env python3
"""Right-arm grasp-geometry diagnostic with non-working-arm masking.
...
class Recorder:
    ...
    def attach(self, env, model):
        ...
        def take_action(action, *args, **kwargs):
            if self.phase == "policy" and env.take_action_cnt >= self.max_actions:
                self.event("diagnostic_action_limit_reached",
                           action_step=int(env.take_action_cnt),
                           diagnostic_max_actions=self.max_actions)
                print(f"[DIAG] reached diagnostic action limit={self.max_actions}; stop after grasp phase.", flush=True)
                raise AuditStop(
                    f"诊断只采集抓取阶段，已达到 {self.max_actions} 个 policy actions；"
                    "这不是任务执行错误。"
                )
            if env.eval_success or env.take_action_cnt >= env.step_lim:
                return original_action(action, *args, **kwargs)
            if self.phase == "policy" and env.take_action_cnt > 0 and env.take_action_cnt % 20 == 0:
                print(f"[DIAG] seed={self.seed} policy_action={env.take_action_cnt}", flush=True)
...
def parent(opts) -> int:
    ...
    run = root / "diagnostics" / ("dp_grasp_geometry_short_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
...
def parse_args():
    ...
    p.add_argument("--diag-max-actions", type=int, default=180,
                   help="Only collect this many policy actions per seed; 180 covers the grasp phase.")
