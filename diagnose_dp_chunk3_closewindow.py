#!/usr/bin/env python3
"""Causal diagnostic: 3-step action chunk + clean boundary observation history + non-working-arm masking.
...
def worker(opts) -> int:
    ...
    try:
        ...
        def monitored_eval(*args, **kwargs):
            ...
            # Causal test: keep the original action chunk length (normally 6),
            # and change ONLY the duplicated observation at chunk boundaries.
            #
            # During a chunk, deploy_policy.eval() already appends the observation
            # after every physical action through model.update_obs(obs). On the next
            # outer policy call, DPRunner.get_action(observation) appends the SAME
            # final frame again. Here we suppress only that duplicate append while
            # preserving all intermediate observations and the original 6-action chunk.
            original_runner_get_action = model.runner.get_action
            ...
            def _dedup_get_action(policy, observation=None):
                if (
                    observation is not None
                    and len(model.runner.obs) > 0
                    and _same_obs(model.runner.obs[-1], observation)
                ):
                    rec.event(
                        "duplicate_boundary_observation_suppressed",
                        action_step=int(getattr(env, "take_action_cnt", 0)),
                    )
                    return original_runner_get_action(policy, None)
                return original_runner_get_action(policy, observation)

            model.runner.get_action = _dedup_get_action

            # Causal test: shorten only the executed action chunk to 3 while
            # keeping the same checkpoint/policy and the same clean boundary
            # observation handling used in the 6-step comparison.
            model.runner.n_action_steps = 3

            rec.event(
                "deployment_intervention",
                runner_n_action_steps=int(model.runner.n_action_steps),
                duplicate_boundary_observation_suppressed=True,
                intermediate_update_obs_preserved=True,
                note="execute first 3 actions from each policy prediction; duplicate boundary observation suppressed",
            )
            print(
                "[DIAG] intervention: n_action_steps=3; suppress duplicate boundary obs",
                flush=True,
            )
...
def parent(opts) -> int:
    ...
    run = root / "diagnostics" / ("dp_chunk3_closewindow_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
